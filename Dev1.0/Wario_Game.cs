...
function WW_Init() //Function to initialize WarioWare on the default minigame.
{
	...
	if(!isObject(%this.brickGroup) && !isObject(BrickGroup_44)) //If nothing exists...
	{
		%this.brickgroup = 	new SimGroup(BrickGroup_44) //Make it.
							{
								name = "\c3Wario Ware\c0";
								bl_id = 44;
							};
	}
	else if(!isObject(%this.brickGroup) && isObject(BrickGroup_44)) //If the minigame doesn't know about it...
		%this.brickGroup = BrickGroup_44; //Make it aware.
	%this.setName(WarioWare); //This is used as a method of seeing if WarioWare is initialized, mostly.
	return true;
}

function WW_RegisterGame(%name, %items, %flags, %execfile) //See documentation.
{
	if(!isObject(WarioWare) || !isObject($WW::GameGroup)) //We have to have WarioWare loaded and the GameGroup created.
	{
...
function WW_RoundEnd()
{
	...
	for(%i = 0; %i < $WW::Mini.numMembers; %i++)
	{
		...
		%client.spawnInArea(_WW_WAITSPAWNA.getPosition(), _WW_WAITSPAWNB.getPosition());
	}
...
function WW_BeginGame(%game)
{
	...
	%msg = %game.msg[getRandom(1, %game.msgs)];
	for(%i = 0; %i < $WW::Mini.numMembers; %i++)
	{
		%client = $WW::Mini.member[%i];
		if(%client.WW_CURRnotPlaying)
